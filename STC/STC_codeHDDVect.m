function [y, cost, alpha, s] = STC_codeHDDVect(x, m, h_hat, rho)
...
path = uint8(inf*ones(2^h, w*min(nBlocks,num)));
...
for i = num:-1:1
    ...
    pathOffset=w*nBlocks*uint32(floor(single(i-1)/single(nBlocks)));
     ...
     if(~mod(single(i),single(nBlocks)) && num > nBlocks)
