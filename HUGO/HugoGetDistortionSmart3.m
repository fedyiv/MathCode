function [ distortion CX CY DX DY] = HugoGetDistortionSmart3(X,Y,dim,T,oldCX,oldDX,oldCY,oldDY,iChanged,jChanged,oldDistortion,sigma,gamma,diff2)
    ...
    for i=-T:T    
        for j=-T:T
            for z=-T:T
                ...
                p(1)=Wc(1)*(CY(i+T+1,j+T+1,z+T+1,1)-CX(i+T+1,j+T+1,z+T+1,1));
                p(2)=Wc(2)*(CY(i+T+1,j+T+1,z+T+1,2)-CX(i+T+1,j+T+1,z+T+1,2));
                p(3)=Wc(3)*(CY(i+T+1,j+T+1,z+T+1,3)-CX(i+T+1,j+T+1,z+T+1,3));
                p(4)=Wc(4)*(CY(i+T+1,j+T+1,z+T+1,4)-CX(i+T+1,j+T+1,z+T+1,4));
                
                p(5)=Wc(5)*(CY(i+T+1,j+T+1,z+T+1,5)-CX(i+T+1,j+T+1,z+T+1,5));
                p(6)=Wc(6)*(CY(i+T+1,j+T+1,z+T+1,6)-CX(i+T+1,j+T+1,z+T+1,6));
                p(7)=Wc(7)*(CY(i+T+1,j+T+1,z+T+1,7)-CX(i+T+1,j+T+1,z+T+1,7));
                p(8)=Wc(8)*(CY(i+T+1,j+T+1,z+T+1,8)-CX(i+T+1,j+T+1,z+T+1,8));
                
          
                

                
                distortion=distortion+sum(p);
...
function [W] = w2(d1,d2,d3,diff2,T)

center=int16((((numel(diff2)/8)^(1/3)-1)/2)-T);

if(abs(d1) <= T  && abs(d2) <= T && abs(d3) <= T)
W=diff2(d1+T+1+center,d1+T+1+center,d1+T+1+center,:);
else
    W=zeros(8,1);
end    
end
