function [  ] = HugoEmulateEmbedding(alpha,dim,T,model_correction,directorySource,directorySG,ext,sigma,gamma)
...
for i=1:N
   ...
   % im1=im1(1:5,1:5);
    ...
    Costs=HugoGetCostsCache(list{i},dim,T,sigma,gamma);
    ...
    elseif(model_correction==1)
            ...
            for ind = bits_to_change
                ...
                Yp=im1(i1,j)+1;
                Ym=im1(i1,j)-1;
                dp=HugoGetDistortion(im1,Yp,dim,T,sigma,gamma);
                dm=HugoGetDistortion(im1,Ym,dim,T,sigma,gamma);
