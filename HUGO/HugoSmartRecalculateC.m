function [C] = HugoSmartRecalculateC(oldD,newD,oldC,iChanged,jChanged,dim,T)
...
if(dim==2)
    ...
    for i=iChanged
        for j=[jChanged-2 jChanged-1 jChanged jChanged+1]
            if(j > m-1||j<1||i<1||i>n ||j>m) continue; end;
            if(abs(oldD(i,j,k))<=T && abs(oldD(i,j+1,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i,j+1,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i,j+1,k)+T+1,k)-1;                
            end
            
            if(abs(newD(i,j,k))<=T && abs(newD(i,j+1,k))<=T)                
                C(newD(i,j,k)+T+1,newD(i,j+1,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i,j+1,k)+T+1,k)+1;                
            end
    ...
    for i=[iChanged-2 iChanged-1 iChanged iChanged+1]       
        for j=jChanged
            ...
            if(abs(newD(i,j,k))<=T && abs(newD(i+1,j,k))<=T)                
                C(newD(i,j,k)+T+1,newD(i+1,j,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i+1,j,k)+T+1,k)+1;
            end
        end
    end
    ...
    for ind=[iChanged-2 jChanged-2;iChanged-1 jChanged-1;iChanged jChanged; iChanged+1 jChanged+1]'
        ...
        j=ind(2);
            ...
            if(abs(newD(i,j,k))<=T && abs(newD(i+1,j+1,k))<=T)                
                C(newD(i,j,k)+T+1,newD(i+1,j+1,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i+1,j+1,k)+T+1,k)+1;
            end
...
elseif(dim==3)
    ...
    for i=iChanged
        for j=[jChanged-3 jChanged-2 jChanged-1 jChanged jChanged+1 jChanged+2]
            ...
            if(abs(newD(i,j,k))<=T && abs(newD(i,j+1,k))<=T && abs(newD(i,j+2,k))<=T)              
                C(newD(i,j,k)+T+1,newD(i,j+1,k)+T+1,newD(i,j+2,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i,j+1,k)+T+1,newD(i,j+2,k)+T+1,k)+1;
            end
    ...
    for i=iChanged
        for j=[jChanged-2 jChanged-1 jChanged jChanged+1 jChanged+2 jChanged+3] 
            if(j < 3||i<1||i>n ||j>m) continue; end;
             if(abs(oldD(i,j,k))<=T && abs(oldD(i,j-1,k))<=T && abs(oldD(i,j-2,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i,j-1,k)+T+1,oldD(i,j-2,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i,j-1,k)+T+1,oldD(i,j-2,k)+T+1,k)-1;
             end
             if(abs(newD(i,j,k))<=T && abs(newD(i,j-1,k))<=T && abs(newD(i,j-2,k))<=T)
    ...
    for i=[iChanged-3 iChanged-2 iChanged-1 iChanged iChanged+1 iChanged+2]       
        for j=jChanged
            if(i>n-2||j<1||i<1||i>n ||j>m) continue; end;
           ...
           if(abs(newD(i,j,k))<=T && abs(newD(i+1,j,k))<=T && abs(newD(i+2,j,k))<=T)
                C(newD(i,j,k)+T+1,newD(i+1,j,k)+T+1,newD(i+2,j,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i+1,j,k)+T+1,newD(i+2,j,k)+T+1,k)+1;
            end
        end
    end
    ...
    for i=[iChanged-2 iChanged-1 iChanged iChanged+1 iChanged+2 iChanged+3]      
        for j=jChanged
            if(i<3||j<1||i<1||i>n ||j>m) continue; end;
           if(abs(oldD(i,j,k))<=T && abs(oldD(i-1,j,k))<=T && abs(oldD(i-2,j,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i-1,j,k)+T+1,oldD(i-2,j,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i-1,j,k)+T+1,oldD(i-2,j,k)+T+1,k)-1;
    ...
    for ind=[iChanged-3 jChanged-3;iChanged-2 jChanged-2;iChanged-1 jChanged-1;iChanged jChanged; iChanged+1 jChanged+1;iChanged+2 jChanged+2]'
        ...
        j=ind(2);
            ...
            if(i>n-2 || j> m-2||j<1||i<1||i>n ||j>m) continue; end;
            ...
            if(abs(newD(i,j,k))<=T && abs(newD(i+1,j+1,k))<=T && abs(newD(i+2,j+2,k))<=T)
                C(newD(i,j,k)+T+1,newD(i+1,j+1,k)+T+1,newD(i+2,j+2,k)+T+1,k)=C(newD(i,j,k)+T+1,newD(i+1,j+1,k)+T+1,newD(i+2,j+2,k)+T+1,k)+1;
            end
  ...
  for ind=[iChanged-2 jChanged-2;iChanged-1 jChanged-1;iChanged jChanged;iChanged+1 jChanged+1;iChanged+2 jChanged+2;iChanged+3 jChanged+3]'
        i=ind(1);
        j=ind(2);
            if(i<3 || j<3||j<1||i<1||i>n ||j>m) continue; end;
            if(abs(oldD(i,j,k))<=T && abs(oldD(i-1,j-1,k))<=T && abs(oldD(i-2,j-2,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i-1,j-1,k)+T+1,oldD(i-2,j-2,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i-1,j-1,k)+T+1,oldD(i-2,j-2,k)+T+1,k)-1;                
            end
    ...
    for ind=[iChanged+2 jChanged-2;iChanged+1 jChanged-1; iChanged jChanged;iChanged-1 jChanged+1; iChanged-2 jChanged+2; iChanged-3 jChanged+3]'
        i=ind(1);
        j=ind(2);
            if(i>n-2 || j<3||j<1||i<1||i>n ||j>m) continue; end;
            if(abs(oldD(i,j,k))<=T && abs(oldD(i+1,j-1,k))<=T && abs(oldD(i+2,j-2,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i+1,j-1,k)+T+1,oldD(i+2,j-2,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i+1,j-1,k)+T+1,oldD(i+2,j-2,k)+T+1,k)-1;
   ...
   for ind=[iChanged-2 jChanged+2;iChanged-1 jChanged+1;iChanged jChanged;iChanged+1 jChanged-1;iChanged+2 jChanged-2;iChanged+3 jChanged-3]'
        i=ind(1);
        j=ind(2);
            if(i<3 || j>m-2||j<1||i<1||i>n ||j>m) continue; end;
            if(abs(oldD(i,j,k))<=T && abs(oldD(i-1,j+1,k))<=T && abs(oldD(i-2,j+2,k))<=T)
                C(oldD(i,j,k)+T+1,oldD(i-1,j+1,k)+T+1,oldD(i-2,j+2,k)+T+1,k)=C(oldD(i,j,k)+T+1,oldD(i-1,j+1,k)+T+1,oldD(i-2,j+2,k)+T+1,k)-1;               
            end
            if(abs(newD(i,j,k))<=T && abs(newD(i-1,j+1,k))<=T && abs(newD(i-2,j+2,k))<=T)
