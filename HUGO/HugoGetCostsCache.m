function [ Costs ] = HugoGetCostsCache(imFileName,dim,T,sigma,gamma)
...
if(exist([costsPath costsFileName '.costs'],'file'))
    %load costss from the file
    
    im=imread(imFileName);
    [sizeX sizeY]=size(im);
    
    fid = fopen([costsPath costsFileName '.costs'], 'r');
    ...
    im=imread(imFileName); 
    Costs=HugoGetCostsSmart(im,dim,T,sigma,gamma);
    Costs2=reshape(Costs,[],1,1);
    h1=int32(mod(sum(Costs2(~isinf(Costs2))),10000));
       
        
    fid = fopen([costsPath costsFileName '.costs'], 'w');
    fwrite(fid,h1,'int32');
