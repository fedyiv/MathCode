function [ Costs ] = HugoGetCostsCache2(imFileName,dim,T,sigma,gamma)
...
load('d:\work\diff.mat')

if(exist([costsPath costsFileName '.costs2'],'file'))
    %load costss from the file
    
    im=imread(imFileName);
    [sizeX sizeY]=size(im);
    
    fid = fopen([costsPath costsFileName '.costs2'], 'r');
    ...
    im=imread(imFileName); 
    Costs=HugoGetCostsSmart2(im,dim,T,sigma,gamma,diff2);
    Costs2=reshape(Costs,[],1,1);
    h1=int32(mod(sum(Costs2(~isinf(Costs2))),10000));
       
        
    fid = fopen([costsPath costsFileName '.costs2'], 'w');
    fwrite(fid,h1,'int32');
