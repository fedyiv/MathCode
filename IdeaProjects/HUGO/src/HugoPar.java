...
public class HugoPar extends Thread{

    public int T,nThreads,iThread;
    public double gamma,sigma;
    public static double [][][] Costs;
    public static double [][][][] C3;
    public static int [][][] D;
    public static int [][]I;

    public HugoPar(int T)
    {
        this(T,4,10);
    }
    public HugoPar()
    ...
    public HugoPar(int T, double gamma, double sigma)
    {

        this(T,gamma,sigma,0,1);
    }

    public HugoPar(int T, double gamma, double sigma,int iThread,int nThreads)
    {
        this.T=T;
        this.gamma=gamma;
        this.sigma=sigma;
...
//D(7) - from bottom left to upper right
    ...
    public double [][][] getCostsSmart3Par(int [][]I)
    {
        int m=I.length;
        int n=I[0].length;
        int i,j;
        ...
        this.C3=getC3(D);
        this.I = I;
        Costs=new double[m][n][2];

        HugoPar []hp =new HugoPar[nThreads];

        for(i = 0; i<nThreads;i++)
        {
                hp[i]=new HugoPar(this.T,this.gamma,this.sigma,i,this.nThreads);
            hp[i].nThreads=nThreads;
            hp[i].T=this.T;
            hp[i].gamma=this.gamma;
            hp[i].sigma=this.sigma;
            hp[i].iThread=i;
        }
    ...
    public double [][][] getCostsSmartPar(int [][]I,int dim)
    {
        double [][][]Costs;
    ...
    public void run()
    {
        ...
        int [][] I1=  ArrayManipulator.copyByVal(this.I);
        int threadLines=m/nThreads;

        for(i=iThread*threadLines;i<(iThread+1)*threadLines;i++)
        for(j=0;j<n;j++)
        {
            I1[i][j]=I[i][j]-1;
            Costs[i][j][0]=getDistortion3Smart(I, I1, D, D, C3, C3, i, j);
            I1[i][j]=I[i][j]+1;
            Costs[i][j][1]=getDistortion3Smart(I, I1, D, D, C3, C3, i, j);
            I1[i][j]=I[i][j];
        }
