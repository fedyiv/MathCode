classdef gopEncoder
    ...
    methods
        function  obj=gopEncoder(gopParam,frames)
           ...
           % for i=1:gopParam(1)
            %    if(obj.gopStructure(i)=='I')
             %       gopBody(i)=frameEncoder(frames(:,:,:,i),'I',-1,-1);
            ...
            %end        
             ...
             for i=obj.gopReorder
                if(obj.gopStructure(i)=='I')
                    gopBody(i)=frameEncoder(frames(:,:,:,i),'I',-1,-1);
                elseif(obj.gopStructure(i)=='P')
                    gopBody(i)=frameEncoder(frames(:,:,:,i),'P',gopBody(obj.previousIP(i)).getY(),-1);
                elseif(obj.gopStructure(i)=='B')
                    gopBody(i)=frameEncoder(frames(:,:,:,i),'B',gopBody(obj.previousIP(i)).getY(),gopBody(obj.nextIP(i)).getY());
                end
             end        
