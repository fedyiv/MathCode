classdef blockDecoder < handle
    ...
    methods
     function obj = blockDecoder(bbs,isIntra,quantiser_scale_code)
            if( nargin > 0)
                 obj.isIntra=isIntra;
                 obj.quantiser_scale_code=quantiser_scale_code;
                
                    
                    obj.fromBitStream(bbs);
                    obj.deQuantize();
    ...
    function deQuantize(obj)
       ...
       if(mod(sum(reshape(obj.blockDCT,1,[])),2)==0)
          ...
          function obj = fromBitStream(obj,bbs)
            ...
            for i=1:k
                blockDCT(floor((i-1)/blockSize)+1,mod(i-1,blockSize)+1)=bbs(i);
            end
            obj.blockDCT=blockDCT-2048;
          end
        
          function obj = blockCompensation(obj,ref)
              obj.blockBody=obj.blockBody+double(ref);              
          end
    end    
end
