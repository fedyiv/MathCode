function [ DivXY DivYX pE] = Model2EvaluateDivergenceOfImage4gridReplacing( directoryCM,ext,dim,T,blockLen,pEmb,visual)
...
for i=1:N
    ...
    cmDir=[directoryCM 'lsbreplace\cm\' fn '\split_' num2str(blockLen) '\'];
    ...
    for p=pEmb
        sgDir=[directoryCM 'lsbreplace\sg\' fn '\split_' num2str(blockLen) '\pEmb_' num2str(p) '\'];
        ...
        disp(['Emulating LSB replacing embedding' num2str(i) ' from ' num2str(N) ]);
        evalc('eurModel3emulateLSBreplacing(p,cmDir,sgDir,ext)');
