function [ DivXY DivYX pE] = Model2EvaluateDivergenceOfImage6e( directoryCM,directoryCM2,ext,dim,T,pEmb,visual,stegosys,amplitude)
...
if(strcmp(stegosys,'lsbmatch'))
    ...
    evalc('eurModel3emulateLSBmatching2(pEmb,cmDir2,sgDir2,ext);');
elseif(strcmp(stegosys,'lsbreplace'))
    disp(['Emulating LSB replacing embedding']);
    evalc('eurModel3emulateLSBreplacing(pEmb,cmDir,sgDir,ext);');
    evalc('eurModel3emulateLSBreplacing(pEmb,cmDir2,sgDir2,ext);');
elseif(strcmp(stegosys,'lsbss'))
    disp(['Emulating LSB SS embedding']);
    evalc('eurModel3emulateSS(pEmb,amplitude,cmDir,sgDir,ext);');
    evalc('eurModel3emulateSS(pEmb,amplitude,cmDir2,sgDir2,ext);');
elseif(strcmp(stegosys,'hugo'))
    disp(['Emulating HUGO embedding']);
    HugoEmulateEmbeddingJ2(pEmb,dim,T,0,cmDir,sgDir,ext,10,4,1,0,0);
    HugoEmulateEmbeddingJ2(pEmb,dim,T,0,cmDir2,sgDir2,ext,10,4,1,0,0);   
end


disp('Calculating features and their divergence');
if(visual=='N')
    [DivXY,DivYX,pE] = Model2EvaluateDivergenceOfImage3( sgDir2,sgDir,ext,dim,T,'N',2);
elseif (visual=='Y')
    [DivXY,DivYX,pE] = Model2EvaluateDivergenceOfImage3( sgDir2,sgDir,ext,dim,T,'M',2);
end
