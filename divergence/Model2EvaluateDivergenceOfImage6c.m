function [ DivXY DivYX pE] = Model2EvaluateDivergenceOfImage6c( directoryCM,directoryCM2,ext,dim,T,pEmb,visual,stegosys,amplitude,sigma,gamma)
...
elseif(strcmp(stegosys,'hugo'))
    sgDir=[directoryCM 'hugo\sg\pEmb_' num2str(pEmb) '\dim' num2str(dim) '\' 'T' num2str(T) '\'  'sigma_' num2str(sigma) '\gamma_' num2str(gamma) '\'];    
else
    assert(0);
end
...
elseif(strcmp(stegosys,'hugo'))
    ...
    HugoEmulateEmbeddingJ2(pEmb,dim,T,0,cmDir,sgDir,ext,sigma,gamma,1,0,0)
