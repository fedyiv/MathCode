function [  ] = eurModel3emulateSS(pEmb,amplitude,directorySource,directorySG,ext)
...
for i=1:N
    ...
    %emb=round(rand(sizeX,sizeY)); % not used in SS, for modeling
    
    for j=1:sizeX
        for k=1:sizeY
            if(mask(j,k)==1)
                im1(j,k)=im1(j,k)+sign(rand()-0.5)*amplitude;
